# Karta w Google Play: gotowe teksty

Wszystko poniżej można skopiować do Play Console → **Rozwój sklepu → Główna strona aplikacji**.
Limity Google: tytuł do 30 znaków, krótki opis do 80, pełny opis do 4000.

## Polski (pl-PL), język domyślny

**Tytuł** (24 znaki)

```
Frost Camp: Obrona Obozu
```

**Krótki opis** (72 znaki)

```
Ścinaj las, sprzedawaj drewno i broń obozu przed potworami w 6 światach!
```

**Pełny opis**

```
Zbuduj obóz ocalałych na końcu świata i obroń go przed hordami potworów!

🪓 ZBIERAJ I BUDUJ
Wirujące topory kosią las w zagrodzie, a ty nosisz na plecach coraz wyższy stos bali. Zanieś drewno na ladę, gdzie kolejka ocalałych zapłaci gotówką. Za pieniądze stawiasz wieże, palisadę, namioty i zatrudniasz drwali oraz górników.

💎 KOPALNIA KLEJNOTÓW
Wydobywaj szmaragdy, diamenty i rzadki obsydian. Za klejnoty ulepszysz w Kuźni plecak, siekierę, kilof, buty i moc wież. Te ulepszenia zostają z tobą na zawsze.

🏹 OBRONA
Co minutę nadciąga fala potworów: szybcy biegacze, opancerzone bestie i złodzieje, którzy rzucają się na twoje pieniądze. Postaw kusze, wieże lodowe, ogniste i armaty, a w Zbrojowni zamień drewniane wieże w kamienne twierdze.

👑 BOSSOWIE
Co 10. fala czeka boss w koronie. Wezwij go, gdy będziesz gotowy. Rozbij toporami jego tarczę, uciekaj z kręgu przed uderzeniem i pokonaj go, zanim się wycofa. Potem zbuduj portal i ruszaj dalej.

🌍 6 ŚWIATÓW
Mroźna Tajga z niedźwiedziami polarnymi, Złote Wydmy ze skorpionami, Dzika Dżungla z gorylami, Mroczne Bagna z krokodylami, Popielny Szczyt z golemami lawy i Kryształowa Kraina z kryształowymi pająkami. Każdy świat ma własną muzykę, drzewa i ocalałych. Po szóstym świecie zaczynasz od nowa na wyższym poziomie trudności, bez końca.

🎁 CODZIENNIE COŚ NOWEGO
Nagrody dzienne z 7-dniową serią, 3 misje dziennie, darmowe skrzynie, skórki kurtki i zwierzaki pomocnicy: lisek, sowa i mały smok.

📴 GRAJ OFFLINE
Gra działa bez internetu, a drwale zarabiają nawet wtedy, gdy nie grasz.

Proste sterowanie jednym palcem, krótkie sesje i długa progresja dla prawdziwych tryhardów. Zbuduj najlepszy obóz!
```

## English (en-US)

**Title** (28 characters)

```
Frost Camp: Survival Defense
```

**Short description** (74 characters)

```
Chop the forest, sell logs and defend your camp from monsters in 6 worlds!
```

**Full description**

```
Build a survivor camp at the edge of the world and defend it from monster hordes!

🪓 COLLECT & BUILD
Spinning axes mow down the fenced forest while you carry an ever-growing stack of logs on your back. Bring the wood to the counter, where a queue of survivors pays in cash. Spend it on towers, walls, tents, lumberjacks and miners.

💎 GEM MINE
Mine emeralds, diamonds and rare obsidian. Use gems at the Forge to upgrade your backpack, axe, pickaxe, boots and tower power. Forge upgrades are yours forever.

🏹 DEFENSE
A monster wave arrives every minute: fast runners, armoured brutes and thieves who go straight for your cash. Build crossbows, ice and fire towers and cannons, and turn wooden towers into stone fortresses at the Armory.

👑 BOSSES
Every 10th wave a crowned boss waits for you. Summon it when you're ready, smash its shield with your axes, dodge its ground slam and beat it before it retreats. Then build the portal and travel on.

🌍 6 WORLDS
Frozen Taiga with polar bears, Golden Dunes with scorpions, Wild Jungle with gorillas, Dark Swamp with crocodiles, Ash Peak with lava golems and Crystal Realm with crystal spiders. Every world has its own music, trees and survivors. After the sixth world you start again on a higher difficulty tier, forever.

🎁 SOMETHING NEW EVERY DAY
Daily rewards with a 7-day streak, 3 daily quests, free chests, jacket skins and helper pets: a fox, an owl and a baby dragon.

📴 PLAY OFFLINE
No internet needed, and your lumberjacks keep earning while you're away.

Simple one-finger controls, short sessions and deep progression for true tryhards. Build the ultimate camp!
```

## Grafiki (folder `store/`)

| Plik | Gdzie w Play Console |
|---|---|
| `icon-512.png` | Ikona aplikacji (512×512) |
| `feature-graphic-1024x500.png` | Grafika promocyjna (1024×500) |
| `screenshot-1-forest.png` … `screenshot-6-shop.png` | Zrzuty ekranu telefonu (1080×1920, min. 2, max. 8) |

Zrzuty mają polskie podpisy. Do wersji angielskiej można wgrać te same pliki.

## Kategoria i tagi

- **Typ aplikacji:** Gra
- **Kategoria:** Strategiczne (alternatywa: Symulacyjne / Casual)
- **Tagi:** Tower defense, Budowanie bazy, Idle, Casual, Offline, Jeden palec
- **E-mail kontaktowy:** ten sam adres co w polityce prywatności
- **Polityka prywatności:** publiczny URL do `docs/privacy-policy.html` (patrz README)

## Zawartość aplikacji (Play Console → Zasady → Zawartość aplikacji)

**Grupa docelowa:** 13–15, 16–17, 18+. Nie zaznaczaj grup poniżej 13 lat.
Gra dla dzieci podlega programowi „Rodzina” i musiałaby mieć inne ustawienia reklam.

**Reklamy:** Tak, aplikacja zawiera reklamy.

**Ankieta klasyfikacji treści (IARC):**
- Kategoria: Gra
- Przemoc: tak, fantastyczna/kreskówkowa przemoc wobec potworów, bez krwi i bez ludzi jako celów
- Strach, seks, wulgaryzmy, narkotyki, hazard za prawdziwe pieniądze: nie
- Interakcja użytkowników / czat: nie
- Udostępnianie lokalizacji: nie
- Zakupy cyfrowe: tak
- Zakupy w grze z losowymi przedmiotami: **tak**. Skrzynia kosztuje 20 szmaragdów, a szmaragdy można też kupić za prawdziwe pieniądze.
  Google wymaga wtedy podania szans na nagrody. Gra pokazuje je w Sklepie → Skrzynie.
- Spodziewany wynik: PEGI 7 / USK 6 / ESRB Everyone 10+

**Aplikacja dotycząca zdrowia / finansów / wiadomości / rządowa:** nie.

**Identyfikator reklamowy:** Tak, używany do reklam (AdMob).

## Bezpieczeństwo danych (Play Console → Zawartość aplikacji → Bezpieczeństwo danych)

Odpowiedzi wynikają z użycia Google AdMob i Google Play Billing. Sama gra nic nie wysyła.

| Pytanie | Odpowiedź |
|---|---|
| Czy aplikacja zbiera lub udostępnia dane użytkownika? | **Tak** (przez AdMob) |
| Czy dane są szyfrowane podczas przesyłania? | **Tak** |
| Czy użytkownik może poprosić o usunięcie danych? | **Tak**. Wskaż polityki Google i kontakt e-mail. Zapis gry usuwa się przy odinstalowaniu. |
| Konto wymagane? | **Nie** |

Zaznacz takie typy danych:

| Typ danych | Zbierane | Udostępniane | Cel | Opcjonalne? |
|---|---|---|---|---|
| **Identyfikatory urządzenia lub inne** (identyfikator reklamowy) | Tak | Tak | Reklamy lub marketing, Statystyki | Nie |
| **Lokalizacja przybliżona** (z adresu IP) | Tak | Tak | Reklamy lub marketing | Nie |
| **Interakcje z aplikacją** | Tak | Tak | Reklamy lub marketing, Statystyki | Nie |
| **Dzienniki awarii / Diagnostyka** | Tak | Nie | Statystyki | Nie |
| **Historia zakupów** | Tak | Nie | Funkcje aplikacji | Nie |

Oficjalne odpowiedzi Google dla AdMob: [developers.google.com/admob/android/privacy/play-data-disclosure](https://developers.google.com/admob/android/privacy/play-data-disclosure).
Przed wysłaniem porównaj z nimi tabelę, bo Google czasem je aktualizuje.
